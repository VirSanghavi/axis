...
var NerveCenter = class _NerveCenter {
  ...
  async getSharedContext() {
    if (!(this.useSupabase && this.supabase && this._projectId) && this.contextManager.apiUrl) {
      try {
        const data = await this.callCoordination(
          `notepad?projectName=${encodeURIComponent(this.projectName)}`
        );
        return { notepad: data?.notepad || "", projectName: this.projectName };
      } catch (e) {
        logger.warn("Failed to fetch notepad from remote API; serving local copy", e);
      }
    }
...
- [JOB CLAIMED] Agent '${agentId}' picked up: ${localJob.title}`);
   ...
   */
  async releaseJob(jobId, force = false, releasedBy) {
    return await this.mutex.runExclusive(async () => {
      const releaseGuard = (status, updatedAt) => {
        if (status !== "in_progress") {
          return { status: "NOT_AVAILABLE", message: "Only in_progress jobs can be released." };
        }
        if (!force && !isReclaimable({ id: jobId, status, updatedAt }, Date.now(), this.lockTimeout)) {
          return { status: "AGENT_ACTIVE", message: "Job's agent still looks active. Pass force: true to release anyway." };
        }
        return null;
      };
      if (this.useSupabase && this.supabase && this._projectId) {
        const { data: job2, error } = await this.supabase.from("jobs").select("id,title,status,assigned_to,updated_at").eq("project_id", this._projectId).eq("id", jobId).maybeSingle();
        if (error) return { status: "ERROR", message: error.message };
        if (!job2) return { status: "NOT_FOUND", message: `Job '${jobId}' was not found.` };
        const denied2 = releaseGuard(job2.status, Date.parse(job2.updated_at));
        if (denied2) return denied2;
        const { data, error: updateError } = await this.supabase.from("jobs").update({
          status: "todo",
          assigned_to: null,
          cancel_reason: `released from ${job2.assigned_to || "unassigned"} by ${releasedBy || "unknown"}`,
          updated_at: (/* @__PURE__ */ new Date()).toISOString()
        }).eq("project_id", this._projectId).eq("id", jobId).eq("status", "in_progress").select("id,title,description,priority,status,assigned_to,dependencies,completion_key,created_at,updated_at").maybeSingle();
        if (updateError) return { status: "ERROR", message: updateError.message };
        if (!data) return { status: "NOT_AVAILABLE", message: `Job '${jobId}' changed state before it could be released.` };
        const released = this.jobFromRecord(data);
        await this.appendToNotepad(`
- [JOB RELEASED] '${released.title}' returned to the board (was ${job2.assigned_to || "unassigned"}).`);
      ...
      if (this.contextManager.apiUrl) {
        try {
          ...
          const released = data?.id ? this.jobFromRecord(data) : data?.job;
          await this.appendToNotepad(`
- [JOB RELEASED] '${released?.title || jobId}' returned to the board.`);
          ...
          return { status: "ERROR", message: `Release failed: ${e.message}` };
        }
      }
...
var TOOL_READ_CONTEXT = "read_context";
var TOOL_UPDATE_CONTEXT = "update_context";
var TOOL_SEARCH_CODEBASE = "search_codebase";
var LOCAL_TOOLS = [
  {
...
var LOCAL_TOOL_NAMES = LOCAL_TOOLS.map((t) => t.name);
...
var READ_CONTEXT_TOOL = TOOL_READ_CONTEXT;
var UPDATE_CONTEXT_TOOL = TOOL_UPDATE_CONTEXT;
var SEARCH_CONTEXT_TOOL = TOOL_SEARCH_CODEBASE;
